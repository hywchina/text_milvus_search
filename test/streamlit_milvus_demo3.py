        ...
        if "error" in res:
            ...
            for i, doc in enumerate(docs, 1):
                ...
                with st.expander(f"Result {i} — {os.path.basename(doc['path'])} (score: {doc['score']:.4f})"):
                    ...
                    # 原文切换按钮（避免嵌套 expander）
                    if doc.get("path") and os.path.isfile(doc["path"]):
                        show_full = st.toggle("查看原文", key=f"show_full_{i}")
                        if show_full:
                            try:
                                with open("/Users/huyanwei/Projects/text_milvus_search/data/三国演义.txt", "r", encoding="utf-8") as f:
                                    content = f.read()
